var drawNetwork = (cx: CanvasRenderingContext2D, inputs: Array<number>, output: number, weights: Array<number>) => {
    cx.font = "16px rcr";

    let arr1: Array<number> = [48, 32, 16];
    ...
    let txt: string = output * 100 + "%";
    cx.fillStyle = "white";
    cx.fillText(txt, 132, 64);
}

var drawData = (cx: CanvasRenderingContext2D, data: Array<Array<number>>): void => {
    for (let x = 0; x < data.length; x++) {
        for (let y = 0; y < data[x].length; y++) {
            cx.fillStyle = "white";
            let nb:string = "0" + data[x][y];
            while (nb.length < 10) {
                nb += " ";
            }
...
window.onload = (): void => {
    ...
    let cx: CanvasRenderingContext2D = canvas.getContext("2d", { alpha: false });
    canvas.width = 512;
    canvas.height = 512;
